...
import { Image as ImageIcon, ImageOff, ArrowLeft, Pencil, CheckSquare, Square, FolderInput, X, RefreshCw, Trash2 } from 'lucide-react';
...
export const ContentGrid: React.FC<ContentGridProps> = ({ 
  ...
  const handleImageClick = (index: number, id: string, e: React.MouseEvent) => {
    // Prevent click if it was a long press event
  ...
  const handleTouchMove = () => {
    // If user moves finger (scroll or drag), cancel long press
    if (longPressTimerRef.current) {
        clearTimeout(longPressTimerRef.current);
    }
  };

  // Wrapper for Viewer Actions
  const handleViewerDelete = (id: string) => {
      if (onDeleteItems) {
          onDeleteItems([id]);
          setViewerOpen(false); // Close viewer after delete
      }
  };

  const handleViewerMove = (id: string, targetCategory: string) => {
      onMoveItems([id], targetCategory);
      setViewerOpen(false); // Close viewer after move
  };
  ...
  return (
    <div className="w-full relative pb-32"> {/* Increased padding bottom for larger action bar */}
        {/* Header with Back Button and Select Toggle */}
        <div className="flex items-center mb-6 justify-between">
            ...
            <div className="flex items-center space-x-2">
                {isInboxMode && onScan && (
                  <button
                      onClick={onScan}
                      className="flex items-center px-3 py-2 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 transition-colors mr-2 border border-indigo-200 shadow-sm"
                  >
                      <RefreshCw size={18} className="mr-2" />
                      <span className="text-sm font-medium hidden sm:inline">Scannen</span>
                  </button>
                )}
                <button
        ...
        {images.length === 0 ? (
           <div className={`flex flex-col items-center justify-center py-20 rounded-2xl border border-dashed
             ...
             <p>Geen afbeeldingen in deze categorie.</p>
           </div>
        ) : (
            /* Compact Grid */
            ...
            {images.map((item, index) => {
                ...
                return (
                    ...
                    onDragStart={(e) => !isSelectionMode && handleDragStart(e, item.id)}
                    ...
                    >
                        ...
                        {/* Meta Data Area */}
                        <div className={`h-1/4 p-3 flex flex-col justify-center border-t relative
                             ${darkMode ? 'bg-black border-stone-800' : 'bg-white border-stone-100'}`}>
                            <h3 className={`text-sm font-medium truncate pr-16 ${darkMode ? 'text-white' : 'text-slate-800'}`} title={item.customName || item.title}>
                                {item.customName || item.title}
                            </h3>
                            <p className={`text-xs mt-1 truncate ${darkMode ? 'text-stone-400' : 'text-slate-500'}`}>
                            {item.size}
                            </p>

                            {/* Actions (Edit/Delete) - Only when not selecting */}
                            {!isSelectionMode && (
                                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    {onDeleteItems && (
                                        <button 
                                            onClick={(e) => handleDeleteClick(e, item.id)}
                                            className="p-1.5 text-slate-400 hover:text-white bg-stone-100/10 hover:bg-red-600 rounded-full transition-colors"
                                            title="Verwijderen"
                                        >
                                            <Trash2 size={12} />
                                        </button>
                                    ...
                                    <button 
                                        ...
                                        className="p-1.5 text-slate-400 hover:text-white bg-stone-100/10 hover:bg-indigo-600 rounded-full transition-colors"
                                        title="Bewerken"
                                    >
                                        <Pencil size={12} />
                                    </button>
        ...
        {/* Selection Action Bar (Sticky Bottom) - LARGE BUTTONS */}
        {isSelectionMode && selectedIds.size > 0 && (
            <div className="fixed bottom-6 left-0 right-0 px-4 flex justify-center z-30">
                <div className="bg-white border border-stone-200 shadow-2xl rounded-2xl flex items-center p-3 sm:p-4 animate-in slide-in-from-bottom-5 text-slate-800">
                    <div className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-lg font-bold mr-4 shadow-sm">
                        {selectedIds.size}
                    </div>
                    ...
                    <div className="relative flex items-center gap-3">
                        <button 
                            ...
                            className="flex items-center space-x-2 bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-xl transition-colors font-medium text-base shadow-sm"
                        >
                            <FolderInput size={22} />
                         ...
                         {onDeleteItems && (
                            <button 
                                ...
                                className="flex items-center space-x-2 bg-red-50 hover:bg-red-100 text-red-600 px-6 py-3 rounded-xl transition-colors font-medium border border-red-200 text-base shadow-sm"
                            >
                                <Trash2 size={22} />
                        ...
                        {/* Dropdown Menu - UPDATED FOR LARGER SIZE */}
                        {isMoveMenuOpen && (
                            <div className="absolute bottom-full mb-4 left-0 w-80 bg-white border border-stone-200 rounded-2xl shadow-2xl overflow-hidden py-1 z-50">
                                <div className="px-5 py-4 text-base font-bold text-slate-600 border-b border-stone-100 mb-1 bg-stone-50">
                                    Kies een map...
                                </div>
                                <div className="max-h-[60vh] overflow-y-auto">
                                    {categories.map(cat => (
                                        <button
                                            ...
                                            disabled={cat === categoryName}
                                            className={`w-full text-left px-6 py-4 text-lg border-b border-stone-50 flex items-center
                                                ${cat === categoryName 
                                                    ? 'text-slate-400 cursor-default bg-stone-50' 
        ...
        {/* Full Screen Image Viewer - Keeps dark mode for optimal viewing */}
        <ImageViewer 
          isOpen={viewerOpen}
          ...
          onDelete={onDeleteItems ? handleViewerDelete : undefined} // Pass delete handler
          onMove={handleViewerMove} // Pass move handler
        />
    </div>
  );
};